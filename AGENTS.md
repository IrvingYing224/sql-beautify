...
README 只写最终用户的安装、使用、配置和风险信息。迁移细节写入 `docs/migration-to-2.1.md`；架构与内部契约写入 `docs/technical/`；不要把阶段性实现计划复制回 README。
...
- 授权边界：适用于仓库内可验证、可回退的规划、实现、测试、审查、委派、文档、暂存、本地合并和本地提交；不授权 push、release、改写历史、删除用户数据或其他不可逆 / 外部状态变更。
