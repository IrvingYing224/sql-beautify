...
## 经验规则：项目内工作无需逐次用户确认
- 触发信号：已明确总体目标和工作范围，后续需要推进计划、checkpoint、代码修改、测试、审查、必要文档或聚焦的本地提交时。
- 根因 / 约束：逐个 checkpoint 等待用户确认会割裂长期任务，也不能替代自动化验证和独立审查；用户已授权主代理在既定项目范围内连续推进。
- 正确做法：主代理可自主制定或调整实施计划、修改仓库文件、运行本地验证、委派边界清晰的子任务、处理审查发现、更新必要文档，并在验证闭合后创建聚焦的本地提交；上述操作均无需用户逐次测试、批准或明确确认。除非出现无法从仓库证据消除的关键歧义、需要扩大既定范围，或必须取得新的外部权限，否则应继续推进直至完成。
- 完成约束：验证失败、证据不足或存在未解决的 Critical / Important finding 时，不得声称完成或创建对应完成提交。用户明确要求暂停或暂不提交时，应遵从该要求。
- 授权边界：该规则不自动授权合并、推送、发布、改写历史、删除用户数据或其他不可逆 / 外部状态变更；这些操作仍需用户明确授权。
- 验证方法：最终交付应列出实际改动、验证命令与结果、审查结论和本地提交信息，并确认未越过授权边界。
- 适用范围：本仓库内的规划、实现、测试、审查、文档维护和本地 Git 提交。
