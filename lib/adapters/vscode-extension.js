var sqlFormatter = require('../core/sql-formatter');
var sqlDialect = require('../core/sql-dialect');
var ddlFormatter = require('../experimental/ddl');
var vscodeConfig = require('./vscode-config');
var rangeFormatPolicy = require('./range-format-policy');
var formatterDiagnostics = require('./formatter-diagnostics');
var safeDiagnosticReport = require('./safe-diagnostic-report');

function create_extension(vscode, dependencies) {
    var deps = dependencies || {};
    var activeSqlFormatter = deps.sqlFormatter || sqlFormatter;
    var activeDdlFormatter = deps.ddlFormatter || ddlFormatter;
    var diagnostics = formatterDiagnostics.create_diagnostics(vscode);

    function get_target_ranges(editor) {
        var selections = [];

        for (var i = 0; i < editor.selections.length; i++) {
            var s = editor.selections[i];
            if (!s.start.isEqual(s.end)) {
                selections.push(new vscode.Range(s.start, s.end));
            }
        }

        if (selections.length === 0) {
            selections.push(new vscode.Range(editor.document.positionAt(0), editor.document.positionAt(editor.document.getText().length)));
        }

        return selections;
    }

    function try_format(formatter, text, phase) {
        try {
            return formatter(text);
        } catch (error) {
            diagnostics.formatter_failed(error, phase);
            return null;
        }
    }

    function report_runtime_diagnostics(result, phase) {
        if (result && result.diagnostics && result.diagnostics.length > 0) {
            diagnostics.runtime_diagnostics(result.diagnostics, phase);
        }
    }

    function format_sql_result(text, document) {
        var config = vscodeConfig.get_sql_formatter_config(vscode, document);

        if (typeof activeSqlFormatter.format_sql_detailed == 'function') {
            return activeSqlFormatter.format_sql_detailed(text, config);
        }

        return {
            text: activeSqlFormatter.format_sql(text, config),
            diagnostics: []
        };
    }

    function replace_target_ranges(formatter, options) {
        var editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
        var behavior = options || {};

        var ranges = get_target_ranges(editor);
        ranges.sort(function(a, b) {
            if (a.start.isBefore(b.start)) {
                return -1;
            }
            if (b.start.isBefore(a.start)) {
                return 1;
            }
            return 0;
        });

        for (var i = 1; i < ranges.length; i++) {
            if (ranges[i - 1].end.isAfter(ranges[i].start)) {
                diagnostics.overlapping_selection();
                return;
            }
        }

        if (behavior.rangeSafety) {
            var rangeConfig = vscodeConfig.get_sql_formatter_config(vscode, editor.document);
            var rangeDialect = sqlDialect.get_capabilities(rangeConfig.dialect);

            for (var k = 0; k < ranges.length; k++) {
                var policyResult = rangeFormatPolicy.analyze_range(editor.document, ranges[k], rangeDialect);
                if (!policyResult.safe) {
                    diagnostics.unsafe_fragment(policyResult.reason, behavior.phase || 'command_format');
                    return;
                }
            }
        }

        editor.edit(function(builder) {
            for (var j = 0; j < ranges.length; j++) {
                var range = ranges[j];
                var text = editor.document.getText(range).toString();
                var formatted = try_format(formatter, text, 'command_format');
                if (formatted !== null) {
                    if (formatted && typeof formatted.text == 'string') {
                        report_runtime_diagnostics(formatted, behavior.phase || 'command_format');
                        builder.replace(range, formatted.text);
                    } else {
                        builder.replace(range, formatted);
                    }
                }
            }
        }).then(function(success) {
            if (!success) {
                diagnostics.rejected_edit();
            }
        });
    }

    function format_sql(text, document) {
        return format_sql_result(text, document).text;
    }

    function get_full_document_range(document) {
        return new vscode.Range(document.positionAt(0), document.positionAt(document.getText().length));
    }

    function register_sql_formatting_providers(context) {
        var selector = ['sql', 'hive-sql'];

        var documentFormatter = vscode.languages.registerDocumentFormattingEditProvider(selector, {
            provideDocumentFormattingEdits: function(document) {
                var formatted = try_format(function(text) {
                    return format_sql_result(text, document);
                }, document.getText(), 'document_format');
                if (formatted === null) {
                    return [];
                }
                report_runtime_diagnostics(formatted, 'document_format');

                return [
                    vscode.TextEdit.replace(get_full_document_range(document), formatted.text)
                ];
            }
        });

        var rangeFormatter = vscode.languages.registerDocumentRangeFormattingEditProvider(selector, {
            provideDocumentRangeFormattingEdits: function(document, range) {
                var rangeConfig = vscodeConfig.get_sql_formatter_config(vscode, document);
                var rangeDialect = sqlDialect.get_capabilities(rangeConfig.dialect);
                var policyResult = rangeFormatPolicy.analyze_range(document, range, rangeDialect);
                if (!policyResult.safe) {
                    diagnostics.unsafe_fragment(policyResult.reason, 'range_format');
                    return [];
                }
                var formatted = try_format(function(text) {
                    return format_sql_result(text, document);
                }, document.getText(range), 'range_format');
                if (formatted === null) {
                    return [];
                }
                report_runtime_diagnostics(formatted, 'range_format');

                return [
                    vscode.TextEdit.replace(range, formatted.text)
                ];
            }
        });

        context.subscriptions.push(documentFormatter);
        context.subscriptions.push(rangeFormatter);
    }

    function activate(context) {
        console.log('Congratulations, your extension "sql-beautify" is now active!');

        function run_format_sql() {
            replace_target_ranges(function(text) {
                var editor = vscode.window.activeTextEditor;
                return format_sql_result(text, editor && editor.document);
            }, {
                rangeSafety: true,
                phase: 'command_format'
            });
        }

        function run_format_hive_ddl() {
            replace_target_ranges(function(text) {
                return activeDdlFormatter.ddl(text);
            });
        }

        function run_extract_hive_ddl() {
            replace_target_ranges(function(text) {
                return activeDdlFormatter.extractddl(text);
            });
        }

        var run_copy_safe_diagnostic_report = safeDiagnosticReport.create_copy_safe_diagnostic_report_command(vscode, {
            sqlFormatter: activeSqlFormatter
        });

        var disposable = vscode.commands.registerCommand('extension.beautifySql', run_format_sql);
        var disposableAlias = vscode.commands.registerCommand('sqlBeautify.formatSql', run_format_sql);

        var disposable2 = vscode.commands.registerCommand('extension.beautifySqlddl', run_format_hive_ddl);
        var disposable2Alias = vscode.commands.registerCommand('sqlBeautify.formatHiveDdl', run_format_hive_ddl);

        var disposable3 = vscode.commands.registerCommand('extension.extractDdl', run_extract_hive_ddl);
        var disposable3Alias = vscode.commands.registerCommand('sqlBeautify.extractHiveDdl', run_extract_hive_ddl);
        var disposable4 = vscode.commands.registerCommand(
            'sqlBeautify.copySafeDiagnosticReport',
            run_copy_safe_diagnostic_report
        );

        context.subscriptions.push(disposable);
        context.subscriptions.push(disposableAlias);
        context.subscriptions.push(disposable2);
        context.subscriptions.push(disposable2Alias);
        context.subscriptions.push(disposable3);
        context.subscriptions.push(disposable3Alias);
        context.subscriptions.push(disposable4);
        register_sql_formatting_providers(context);
    }

    function deactivate() {
    }

    return {
        activate: activate,
        deactivate: deactivate
    };
}

exports.create_extension = create_extension;
