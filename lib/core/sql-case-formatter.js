...
function build_case_formatted_text(prefix_raw, case_operand, when_items, else_value, suffix_text, caseWhenThenWrapLength, else_leading_comments, else_trailing_comment) {
	...
	for (let i = 0; i < when_items.length; i++) {
		...
		if (when_items[i].then_trailing_comment && when_items[i].then_trailing_comment != '') {
			...
			lines.push(when_line);
			lines.push(format_case_branch_value(value_indent, 'THEN', normalize_case_value_text(when_items[i].then_text)));
		} else {
...
function format_case_expression_line(line, caseWhenThenWrapLength) {
...
function format_case_blocks(str, caseWhenThenWrapLength) {
	...
	for (let i = 0; i < text_list.length; i++) {
		...
		var single_line_formatted = format_case_expression_line(current_line, caseWhenThenWrapLength);
		...
		var formatted_block = build_case_formatted_text(prefix_raw, case_operand, when_items, else_value, end_suffix, caseWhenThenWrapLength, else_leading_comments, else_trailing_comment);
