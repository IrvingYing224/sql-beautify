...
function case_has_comments(document, caseNode) {
	var startIndex = caseNode.caseKeywordToken ? caseNode.caseKeywordToken.index : -1;
	var endIndex = caseNode.endKeywordToken ? caseNode.endKeywordToken.index : -1;
...
function push_compact_case_token(tokens, token) {
	if (token) {
		tokens.push(token);
	}
}
...
function compact_case_tokens(caseNode) {
	var tokens = [];
	push_compact_case_token(tokens, caseNode.caseKeywordToken);
	for (var b = 0; b < (caseNode.branches || []).length; b++) {
		var branch = caseNode.branches[b];
		push_compact_case_token(tokens, branch.whenKeywordToken);
		tokens = tokens.concat(branch.whenTokens || []);
		push_compact_case_token(tokens, branch.thenKeywordToken);
	...
	push_compact_case_token(tokens, caseNode.endKeywordToken);
	return tokens;
}

function compact_case_text(document, caseNode, config) {
	return sqlTokenRenderer.render_tokens(document, compact_case_tokens(caseNode), {
		applyKeywordCase: true,
		keywordCase: config && config.keywordCase,
		spaceBeforeInParen: true,
		preserveCommaGapExceptFunctionName: 'COALESCE',
		compactOperatorToken: is_originally_compact_case_function_plus,
		followsCompactOperator: follows_originally_compact_case_function_plus,
...
function can_compact_case(document, nodes, caseNode, config, wrapValues) {
	...
	if (caseNode.startLine != caseNode.endLine) {
		return false;
	}
	if (is_nested_case_node(document, caseNode)
		|| case_has_comments(document, caseNode)
		|| case_has_multiline_child_scope(document, caseNode)
		|| wrapValues) {
		return false;
	}
