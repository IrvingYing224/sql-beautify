...
function is_query_paren(tokens, index, dialect) {
    var previous = sqlClauseContext.previous_code_token(tokens, index);
    var next = sqlClauseContext.next_code_token(tokens, index);
...
function is_multiline_query_paren(source, tokens, index) {
    var close_index = sqlClauseContext.find_matching_paren(tokens, index);
    if (close_index < 0) {
        return false;
    }
...
function should_apply_clause_match(tokens, index, clause_match, context) {
    ...
    if (clause && clause.name == 'WITH' && context.lastClause == 'GROUP') {
        previous = sqlClauseContext.previous_code_token(tokens, index);
        next = sqlClauseContext.next_code_token(tokens, index);
        if (previous != null
...
function split_clauses(text, dialect, context) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        var token = tokens[i];
        var next = sqlClauseContext.next_code_token(tokens, i);
