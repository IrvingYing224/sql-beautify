...
function order_comment(str, maxAlignWidth, tokenizer_options){
	...
	function get_code_paren_delta(code) {
		var tokens = sqlTokenizer.tokenize(code, tokenizer_options);
		var delta = 0;
	...
	function get_line_group_key(line_info, before_paren_depth, before_case_depth) {
		...
		var line_case_delta = get_case_balance_delta(code, tokenizer_options);
	...
	function flush_group() {
		...
		for (let i = 0; i < current_group.length; i++) {
			...
			var original_comment = sqlLineModel.from_text(text_list[item.index], tokenizer_options)[0].comment;
	...
	for (let i = 0; i < text_list.length; i++){
		...
		case_depth += get_case_balance_delta(code, tokenizer_options);
		if (case_depth < 0) {
			case_depth = 0;
		}
	}
