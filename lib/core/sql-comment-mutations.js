...
function apply_comment_alignment_mutations(document, nodes, mutations, config) {
	...
	function is_select_item_field_comment_line(lineIndex) {
		var item = select_item_for_line(lineIndex);
		return item
			&& (item.startLine == lineIndex || widthContext.is_case_end_alias_comment_line(lineIndex));
	}

	function current_group_is_select_field_comment_group() {
		if (!current_group_is_select_only()) {
			return false;
		}
		for (var i = 0; i < group.length; i++) {
			if (!is_select_item_field_comment_line(group[i].index)) {
				return false;
			}
		}
		return true;
	...
	function select_span_by_id(ownerScopeId) {
	...
	function is_select_header_modifier_comment_line(lineIndex) {
		var items = nodes && nodes.selectItems ? nodes.selectItems : [];
		var commentItem = null;
		for (var i = 0; i < items.length; i++) {
			if (lineIndex < items[i].startLine
				|| lineIndex > items[i].endLine
				|| items[i].ownerKind != 'selectList') {
				continue;
			}
			if (items[i].endLine == lineIndex
				&& (!commentItem || items[i].endTokenIndex > commentItem.endTokenIndex)) {
				commentItem = items[i];
			}
		}
		var span = commentItem ? select_span_by_id(commentItem.ownerScopeId) : null;
		return !!(span && span.header && span.header.modifier);
	}

	function group_has_select_header_modifier_comment() {
		for (var i = 0; i < group.length; i++) {
			if (is_select_header_modifier_comment_line(group[i].index)) {
				return true;
			}
		}
		return false;
	...
	function flush_group() {
		...
		if (current_group_is_select_field_comment_group() && group.length > 1 && !group_has_case_branch_value_comment()) {
			pendingSelectGroup = group.slice();
		}

		var target = 0;
		var paddingWidth = group_has_select_header_modifier_comment() ? 2 : 1;
		for (var i = 0; i < group.length; i++) {
			if (group[i].alignmentWidth >= maxAlignWidth
				&& !widthContext.is_case_end_alias_comment_line(group[i].index)) {
				continue;
			}
			var width = group[i].codeWidth + paddingWidth;
			if (width > target) {
				target = width;
			}
		}
			...
			if (condition_segment_keyword(line.index) == 'HAVING' && pendingSelectGroup.length > 0 && group.length == 0) {
				...
				if (pendingSelectGroup.length > 0
					...
					&& (!line_index_inside_case_expression(line.index)
						|| widthContext.is_case_end_alias_comment_line(line.index))) {
