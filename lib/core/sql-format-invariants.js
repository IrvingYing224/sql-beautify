...
function select_item_for_token(nodes, token) {
	var items = nodes && nodes.selectItems ? nodes.selectItems : [];

	for (var i = 0; i < items.length; i++) {
		if (token.index >= items[i].startTokenIndex && token.index <= items[i].endTokenIndex) {
			return items[i];
		}
	}
...
function token_omission_has_structural_replacement(document, nodes, plan, token) {
	var item = select_item_for_token(nodes, token);
	return select_item_has_structural_replacement(document, plan, item);
}
...
function assert_comment_alignment_widths_match_renderer(document, nodes, plan, config) {
	...
	for (key in (plan.commentAlignments || {})) {
		...
		if (typeof mutation.plannedCodeWidth != 'number'
			&& typeof mutation.plannedAlignmentWidth != 'number'
			&& typeof mutation.plannedJoinPrefixWidth != 'number') {
			continue;
		}
		if (!widthContext) {
			widthContext = sqlRenderWidth.create_width_context(document, nodes, plan, config || {});
		}
		if (typeof mutation.plannedCodeWidth == 'number'
