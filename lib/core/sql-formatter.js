...
function sanitize_range_for_metadata(range) {
    var source = range || {};

    return {
        start: typeof source.start == 'number' ? source.start : -1,
        end: typeof source.end == 'number' ? source.end : -1
    };
}

function sanitize_unsupported_segment_for_metadata(segment) {
    var source = segment || {};
    ...
    return {
        ...
        code: source.code,
        label: source.label,
        range: sanitize_range_for_metadata(source.range),
...
function sanitize_diagnostic_for_metadata(diagnostic) {
    var source = diagnostic || {};
    var unsupportedSegments = [];
    var segments = source.unsupportedSegments || [];

    for (var i = 0; i < segments.length; i++) {
        unsupportedSegments.push(sanitize_unsupported_segment_for_metadata(segments[i]));
    }
...
function collect_unsupported_diagnostics_metadata(context) {
    var diagnostics = collect_unsupported_diagnostics(context);
    var sanitized = [];

    for (var i = 0; i < diagnostics.length; i++) {
        sanitized.push(sanitize_diagnostic_for_metadata(diagnostics[i]));
    }
...
function format_sql_detailed(text, options) {
    ...
    try {
        ...
        if (includeTelemetry) {
            finalTelemetry = finish_telemetry(telemetry);
            error.sqlBeautifyDiagnostics = collect_unsupported_diagnostics_metadata(context);
