...
function apply_keyword_case_mutations(document, mutations, useUpperKeywordCase, tokenizer_options) {
    var tokens = document.tokens || [];
    var in_set_payload = false;
    var keyword_lookup = build_keyword_lookup(tokenizer_options || document.tokenizerOptions);
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (!tokens[i].isCode || tokens[i].type != 'word') {
            continue;
        }

        if (/^IF$/i.exec(tokens[i].value) && is_if_not_exists(tokens, i)) {
            sqlFormatMutations.add_token_replacement(
                mutations,
                tokens[i].id,
                useUpperKeywordCase === false ? tokens[i].value.toLowerCase() : tokens[i].value.toUpperCase()
            );
            continue;
        }

        if (keyword_lookup[String(tokens[i].value || '').toUpperCase()] !== true) {
            continue;
        }

        if (sqlFormatMutations.get_for_token(mutations, tokens[i].id).replacement) {
            continue;
        }
