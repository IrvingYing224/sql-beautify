...
function add_query_scope_body_mutations(document, scope, mutations, config, clauses) {
	...
	if (compactScope) {
		// Mark compact query scopes as synthetic multiline so inline-query spacing is not reused after layout expands them.
		scope.closeLine = openToken.line + 1;
	}
}
...
function apply_scope_layout_mutations(document, nodes, mutations, config) {
	...
	for (var i = 0; i < scopes.length; i++) {
		...
		add_query_scope_body_mutations(document, scope, mutations, config, clauses);
		...
		} else if (closeToken && scope.closeLine != scope.openLine) {
