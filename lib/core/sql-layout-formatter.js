...
function is_inline_subquery_exempt(document, openToken) {
	var previous = previous_code_token(document, openToken);
...
function apply_scope_layout_mutations(document, nodes, mutations, config) {
	...
	for (var i = 0; i < scopes.length; i++) {
		...
		var token = token_by_index(document, scope.openTokenIndex);
		...
		var firstBodyToken = next_code_token(document, token);
		...
		var closeToken = typeof scope.closeTokenIndex == 'number'
			? token_by_index(document, scope.closeTokenIndex)
			: null;
		...
		var afterCloseToken = closeToken ? next_code_token(document, closeToken) : null;
