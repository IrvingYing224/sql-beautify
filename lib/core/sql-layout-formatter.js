...
function add_minimum_line_indent(mutations, lineIndex, indentText) {
	var existing = mutations.lineIndents[String(lineIndex)];
	if (existing
		&& expand_tabs_for_width(existing.indentText).length > expand_tabs_for_width(indentText).length) {
		return;
	}
	sqlFormatMutations.add_line_indent(mutations, lineIndex, indentText);
}
...
function add_query_scope_body_mutations(document, scope, mutations, config, clauses) {
	...
	if (compactScope && selectToken) {
		...
		var selectItemMutation = selectItemToken
			? sqlFormatMutations.get_for_token(mutations, selectItemToken.id)
			: null;
		if (is_scope_body_token(scope, selectItemToken, directBodyTokenLookup)
			&& !(selectItemMutation && (selectItemMutation.spacingBefore || selectItemMutation.lineBreakBefore))) {
	...
	for (var i = 0; i < scopes.length; i++) {
		...
		if (closeToken && has_code_before_token(document, closeToken)) {
			if (scope.closeLine != scope.openLine && closeToken.line != token.line) {
				add_minimum_line_indent(mutations, closeToken.line, indents.bodyIndent);
			}
