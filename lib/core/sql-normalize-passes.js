...
function normalize_set_payload(payload, tokenizerOptions) {
	var tokens = sqlTokenizer.tokenize(payload.replace(/^\s+/ig, '').replace(/\s+$/ig, ''), tokenizerOptions);
	var text = '';
...
function protect_set_payloads(str, context, tokenizerOptions) {
	var tokens = sqlTokenizer.tokenize(str, tokenizerOptions);
	var text = '';
	...
	for (var i = 0; i < tokens.length; i++) {
		if (tokens[i].type == 'word' && /^SET$/i.exec(tokens[i].value) && at_statement_start) {
			...
			text += context.store('set_payload', normalize_set_payload(payload_text, tokenizerOptions));
