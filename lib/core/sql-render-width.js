...
function create_width_context(document, nodes, mutations, config) {
	...
	function planned_prefix_width(lineIndex) {
		var prefix = planned_line_prefix(lineIndex);
		var line = document.lines[lineIndex];
		var originalIndent = line ? String(line.raw || '').match(/^\s*/)[0] : '';

		if (!prefix) {
			return 0;
		}

		return expand_tabs_for_width(prefix.indentText + prefix.text).length
			- expand_tabs_for_width(originalIndent).length;
	}

	function planned_line_prefix(lineIndex) {
		var line = document.lines[lineIndex];
		var originalIndent = line ? String(line.raw || '').match(/^\s*/)[0] : '';
		var moves = movedSeparatorsByLine[String(lineIndex)] || [];
		var indentText = originalIndent;
		var text = '';
		...
		for (var i = 0; i < moves.length; i++) {
			text += String(moves[i].target.text || '');
			if (i == 0 && typeof moves[i].target.indentText == 'string') {
				indentText = moves[i].target.indentText;
			}
		}
	...
	function apply_planned_line_prefix(lineIndex, text) {
		var prefix = planned_line_prefix(lineIndex);
		...
		originalIndent = source.match(/^\s*/)[0];
		body = source.slice(originalIndent.length);
		return prefix.indentText + prefix.text + body.replace(/^\s+/g, '');
	}

	function apply_planned_line_prefix_to_segments(lineIndex, segments) {
		var result = (segments || []).slice();
		if (result.length == 0) {
			return result;
		}
		result[0] = apply_planned_line_prefix(lineIndex, result[0]);
		return result;
	}
	...
	function planned_unjoined_code_width(line) {
		...
		if (rendered.includesIndent) {
			if (is_case_branch_value_comment_line(line.index)) {
				return max_segment_width(apply_planned_line_prefix_to_segments(line.index, rendered.segments));
			}
			return expand_tabs_for_width(rendered.text).length;
		}

		return expand_tabs_for_width(planned_code_segment(line)).length;
	}
	...
	function planned_code_segment(line) {
		...
		if (indent) {
			segment = indent.indentText + rendered.text;
		} else {
			segment = String(line.raw || '').match(/^\s*/)[0] + rendered.text;
		}

		return apply_planned_line_prefix(line.index, segment);
	}
	...
	function planned_alignment_width(line) {
		...
		if (rendered.includesIndent) {
			if (is_case_branch_value_comment_line(line.index)) {
				return max_segment_alignment_width(apply_planned_line_prefix_to_segments(line.index, rendered.segments));
			}
