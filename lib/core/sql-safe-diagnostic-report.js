...
function normalize_telemetry(telemetry) {
    ...
    var phases = [];
    var i;
    var sourcePhases = safe_array(source.phases);

    for (i = 0; i < sourcePhases.length; i++) {
        phases.push(normalize_phase(sourcePhases[i]));
    }
...
function normalize_diagnostic_summary(diagnostic) {
    return {
        ...
        labels: unique_sorted(safe_array(diagnostic && diagnostic.labels).map(safe_label)),
        sources: unique_sorted(safe_array(diagnostic && diagnostic.sources).map(safe_source)),
...
function normalize_diagnostics(diagnostics) {
    var output = [];
    var items = safe_array(diagnostics);
    var i;
    var j;
    var segments;
    var labels;
    ...
    for (i = 0; i < items.length; i++) {
        segments = safe_array(items[i] && items[i].unsupportedSegments);
        labels = [];
        sources = [];
...
function normalize_classification(value) {
    ...
    return has_own(CLASSIFICATIONS, normalized) ? normalized : 'unknown';
}
