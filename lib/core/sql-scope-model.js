...
function active_tokens(document) {
	...
	for (var i = 0; i < document.tokens.length; i++) {
		if (is_code_token(document.tokens[i])) {
...
function scope_by_id(scopes, scopeId) {
...
function condition_base_indent(scopes, document, scope) {
	...
	var parent = scope_by_id(scopes, scope.parentScopeId);
...
function is_condition_keyword(token) {
	return is_word(token) && CONDITION_KEYWORDS[token.value.toUpperCase()];
}
...
function build(document, options) {
	...
	for (var i = 0; i < tokens.length; i++) {
		...
		if (is_condition_keyword(token) && can_start_condition_at_current_scope(state)) {
