...
function split_top_level_items(text) {
	...
	var tokens = sqlTokenizer.tokenize(source);
	var items = [];
	var paren_depth = 0;
	var start = 0;
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		if (tokens[i].type == 'punctuation' && tokens[i].value == ')' && paren_depth > 0) {
			paren_depth -= 1;
			continue;
		}
...
function extract_leading_standalone_comment_markers(text) {
	var source = String(text || '');
	var tokens = sqlTokenizer.tokenize(source);
	var markers = [];
	var cursor = 0;
...
function append_select_item_lines(lines, prefix, item_text) {
	var extracted = extract_leading_standalone_comment_markers(item_text);
...
function format_select_clause_line(line, keyword, continuation_indent) {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (append_select_item_lines(lines, prefix, items[i])) {
			actual_item_count += 1;
		}
	}
...
function format_select_clause_lists(str) {
	...
	for (let i = 0; i < text_list.length; i++) {
		if (pending_leading_comma) {
			...
			var pending_extracted = extract_leading_standalone_comment_markers(pending_trimmed);
			if (in_select_list
			...
			} else if (in_select_list && pending_trimmed != '' && !/^,/.exec(pending_trimmed) && !is_select_block_end(pending_trimmed, 'generic')) {
		...
		if (in_select_list && /^,/.exec(trimmed)) {
			var continuation_items = split_top_level_items(trimmed.slice(1));
			var emitted_actual_item = false;
			for (let j = 0; j < continuation_items.length; j++) {
				if (append_select_item_lines(output, continuation_indent + ',', continuation_items[j])) {
