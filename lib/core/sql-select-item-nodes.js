...
function is_select_modifier_item(span, itemTokens) {
	if (!span
		|| span.kind != 'selectList'
		|| itemTokens.length != 1
		|| itemTokens[0].type != 'word'
		|| itemTokens[0].line != span.startLine) {
		return false;
	}

	return /^(DISTINCT|ALL)$/i.exec(itemTokens[0].value) != null;
}
