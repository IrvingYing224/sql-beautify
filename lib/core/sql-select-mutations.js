...
function rendered_item_width_before_as(document, nodes, item, mutations) {
	...
	var width = sqlListLayoutPolicy.item_indent(document, nodes, item).length;
	var beforeAsTokens = [];
	for (var i = 0; i < (item.tokens || []).length; i++) {
...
function max_rendered_item_width_before_as(document, nodes, item, mutations) {
	...
	if (caseScope && asToken && asToken.index > caseScope.endTokenIndex) {
		var caseIndent = sqlListLayoutPolicy.item_indent(document, nodes, item);
...
function rendered_item_width_without_as(document, nodes, item) {
	return sqlListLayoutPolicy.item_indent(document, nodes, item).length + render_node_tokens(document, item.tokens || []).length;
}
...
function has_select_header_comment_line(document, nodes, item) {
	...
	if (span.header && span.header.modifier) {
		return false;
	}
	var line = document.lines[span.startLine];
	var selectToken = span.header ? token_by_id(document, span.header.selectTokenId) : null;
	if (!line
		|| !line.hasTrailingComment
		|| !selectToken
		|| selectToken.line != span.startLine
		|| selectToken.type != 'word'
		|| !/^SELECT$/i.exec(selectToken.value)) {
		return false;
	}
	for (var i = 0; i < line.codeTokens.length; i++) {
		if (line.codeTokens[i].index > selectToken.index) {
			return false;
		}
	}
	return true;
...
function apply_multiline_function_item_mutations(document, nodes, mutations, item, config) {
	...
	var itemIndent = sqlListLayoutPolicy.structured_list_indent(document, nodes, item.ownerScopeId, item.ownerKind);
...
function should_join_select_header_first_item(document, nodes, item) {
	...
	var span = select_span_by_id(nodes, item.ownerScopeId);
	if (span && span.header && span.header.modifier) {
		return false;
	}
...
function token_by_id(document, tokenId) {
	var tokens = document && document.tokens ? document.tokens : [];
...
function first_item_for_span(nodes, span) {
	...
	for (var i = 0; i < items.length; i++) {
		...
		if (sqlListLayoutPolicy.is_first_item_in_owner(nodes, items[i])) {
			return items[i];
		}
	}
	return null;
...
function can_join_select_header_modifier_line(document, headerLineIndex, modifierLineIndex) {
	var headerLine = document.lines[headerLineIndex];
	if (!headerLine || headerLine.hasTrailingComment) {
		return false;
	}
	return modifierLineIndex == headerLineIndex + 1;
}
...
function apply_select_header_modifier_mutations(document, nodes, mutations) {
	...
	for (var i = 0; i < spans.length; i++) {
		...
		var headerLineIndex = span.startLine;
		var modifierToken = token_by_id(document, span.header.modifier.tokenId);
		var modifierLineIndex = modifierToken ? modifierToken.line : headerLineIndex;

		if (!firstItem) {
			continue;
		}

		var firstToken = firstItem.tokens && firstItem.tokens[0];
		if (!firstToken) {
			continue;
		}

		var itemIndent = sqlListLayoutPolicy.item_indent(document, nodes, firstItem);
		var modifierJoinedToHeader = false;

		if (modifierToken && modifierToken.line == headerLineIndex) {
			sqlFormatMutations.add_spacing_before_token(mutations, modifierToken.id, ' ');
		}

		if (modifierToken
			&& modifierToken.line > headerLineIndex
			&& can_join_select_header_modifier_line(document, headerLineIndex, modifierToken.line)) {
			modifierJoinedToHeader = true;
			sqlFormatMutations.add_line_join(mutations, modifierToken.line, ' ');
		}

		if (modifierToken
			&& modifierToken.line > headerLineIndex
			&& !modifierJoinedToHeader) {
			sqlFormatMutations.add_line_indent(
				mutations,
				modifierToken.line,
				sqlListLayoutPolicy.list_base_indent(document, nodes, span.id)
			);
		}

		if (firstToken.line == modifierLineIndex) {
		...
		if (firstToken.line > headerLineIndex) {
			sqlFormatMutations.add_line_indent(mutations, firstItem.startLine, itemIndent);
		}
	}
}
...
function apply_group_by_extension_mutations(document, nodes, mutations) {
	...
	for (var i = 0; i < tokens.length; i++) {
		...
		var leadingComma = previous
			...
			? previous
			: null;
		var indent = sqlListLayoutPolicy.structured_list_indent(document, nodes, span.id, span.kind) + ' ';
