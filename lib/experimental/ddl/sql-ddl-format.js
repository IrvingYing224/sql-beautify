...
function is_ddl_type_keyword(value) {
    return /^(array|bigint|boolean|date|datetime|decimal|double|float|int|map|string|struct|text|timestamp|tinyint|varchar)$/i.exec(value);
}

function is_ddl_type_separator(token) {
    if (!token) {
        return false;
    }
    ...
    return token.type == 'operator' && /^[<>:]+$/.exec(token.value);
}
...
function next_significant_token(tokens, index) {
...
function should_skip_ddl_type_space(tokens, index) {
    var prev = previous_significant_token(tokens, index);
    var next = next_significant_token(tokens, index);

    if (!prev || !next) {
        return true;
    }

    if (is_ddl_type_separator(prev) || is_ddl_type_separator(next)) {
        return true;
    }

    return prev.type == 'word'
        && /^DECIMAL$/i.exec(prev.value)
        && next.type == 'punctuation'
        && next.value == '(';
}

function normalize_ddl_type(text) {
    var tokens = sqlTokenizer.tokenize(String(text || ''));
    var output = '';

    for (var i = 0; i < tokens.length; i++) {
        if (tokens[i].type == 'whitespace' || tokens[i].type == 'newline') {
            if (!should_skip_ddl_type_space(tokens, i) && output.slice(-1) != ' ') {
                output += ' ';
            }
            continue;
        }

        if (tokens[i].type == 'word' && is_ddl_type_keyword(tokens[i].value)) {
            output += tokens[i].value.toUpperCase();
            continue;
        }
...
function parse_ddl_column(item) {
    var clean = String(item || '').replace(/^\s*,\s*/, '').replace(/^\s+|\s+$/g, '');
