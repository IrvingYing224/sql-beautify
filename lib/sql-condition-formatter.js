...
function find_root_case_start_loc(line) {
	var tokens = sqlCaseUtils.get_case_tokens(line);
...
function wrap_condition_expression(text) {
	var parts = split_code_and_line_comment(sqlTokenizer.tokenize(text));
...
function is_condition_continuation_line(line) {
	var token = first_code_token(sqlTokenizer.tokenize(line));
...
function line_starts_condition_or_connector(line, dialect) {
	...
	return sqlClauseRegistry.get_condition_clause(trimmed, dialect) != null
		|| is_condition_continuation_line(trimmed);
}
...
function wrap_condition_clauses(str, dialect) {
	...
	var active_dialect = dialect || 'generic';
	...
	for (let i = 0; i < text_list.length; i++) {
		...
		if (sqlClauseRegistry.get_condition_clause(trimmed, active_dialect) != null) {
			...
			output.push(wrap_condition_expression(text_list[i]));
		} else if (in_condition_block && is_condition_continuation_line(text_list[i])) {
			output.push(wrap_condition_expression(text_list[i]));
		} else {
...
function build_condition_line(prefix_tabs, target_keyword_end, keyword, suffix_text) {
	...
	return prefix_tabs + repeat_space(indent_length) + keyword + suffix_text;
}
